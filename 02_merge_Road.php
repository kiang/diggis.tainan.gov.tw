...
foreach(glob(__DIR__ . '/raw/Road/*.json') AS $jsonFile) {
  ...
  foreach($json['features'] AS $f) {
    if(!isset($f['attributes']['WIDTH']) || $f['attributes']['WIDTH'] > 3) {
      continue;
    }
    $feature = array(
      'type' => 'Feature',
      'properties' => array(
        '區域' => $f['attributes']['TOWN'],
        '路名' => "{$f['attributes']['ROADNAME']}{$f['attributes']['RDNAMESECT']}{$f['attributes']['RDNAMELANE']}{$f['attributes']['RDNAMENON']}",
...
file_put_contents(__DIR__ . '/topojson/Road.geo.json', json_encode($fc));
